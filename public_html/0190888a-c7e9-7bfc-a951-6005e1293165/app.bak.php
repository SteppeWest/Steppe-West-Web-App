...
function init() {
	global $updated, $pageId, $canonical, $languages, $currentLanguage, $languageData;
	...
	// Extracting the subdomain and the path
	$parsedUrl = parse_url($url);
	$host = explode('.', $parsedUrl['host']);
	$path = isset($parsedUrl['path']) ? trim($parsedUrl['path'], '/') : '';

	$subdomain = array_shift($host); // Get the subdomain

	// Check the first part of the path
	$pathParts = explode('/', $path);
	$firstPathPart = isset($pathParts[0]) ? $pathParts[0] : '';
	$secondPathPart = isset($pathParts[1]) ? $pathParts[1] : '';

	if ($subdomain == 'steppewest') {
		if ($path == '' || in_array($firstPathPart, $languages)) {
			$pageId = 'home';
		}
		else {
			$pageId = 'error';
	...
	elseif ($subdomain == 'invite') {
		if ($path == '' || in_array($firstPathPart, $languages)) {
			$pageId = 'invite';
		}
		elseif ($firstPathPart == 'faq' && in_array($secondPathPart, $languages)) {
			$pageId = 'faq';
		}
		else {
			$pageId = 'error';
	...
	$metaData = fetchData('data/meta/' . $currentLanguage, true);
	$contentData = fetchData('data/' . $pageId . '/' . $currentLanguage, true);

	// Merge shared and content data
	$languageData = array_merge($commonData, $metaData, $contentData);
